    ...
    def MoveToItem(self, item):
        ...
        if self.InReportView():
            ...
            if not self.HasAGWFlag(ULC_HAS_VARIABLE_ROW_HEIGHT):
                ...
                if rect.y < view_y:
                    self.Scroll(int(-1), int(rect.y/hLine))
                if rect.y+rect.height+5 > view_y+client_h:
                    self.Scroll(int(-1), int((rect.y+rect.height-client_h+hLine)/hLine))
                ...
                if start_y < view_y:
                    ...
                    self.Scroll(int(-1), int(start_y//SCROLL_UNIT_Y))

                if start_y + height > view_y + client_h:
                    while start_y + height < view_y + client_h:
                        start_y += SCROLL_UNIT_Y

                    self.Scroll(int(-1), int((start_y+height-client_h+SCROLL_UNIT_Y)//SCROLL_UNIT_Y))
...
# ----------------------------------------------------------------------------
    ...
    def ScrollList(self, dx, dy):
        ...
        if not self.HasAGWFlag(ULC_HAS_VARIABLE_ROW_HEIGHT):
            ...
            self.Scroll(int(-1), int(top + dy/hLine))
        else:
            self.Scroll(int(-1), int(top + dy/SCROLL_UNIT_Y))
